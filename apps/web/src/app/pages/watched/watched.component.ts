...
@Component({
    ...
    <div class="page">
      <div class="page-header">
        <div>
          ...
          <p class="subtitle">Filmes que você marcou como assistido</p>
        </div>
      ...
      @if (loading) {
        ...
        <div class="card empty-state">
          ...
          <p class="empty-title">Nenhum filme assistido ainda</p>
          <p class="empty-text">Busque um filme e clique em "Assistido".</p>
        </div>
      } @else {
  ...
  styles: [`
    ...
    .count-badge { background: rgba(210,224,3,0.18); color: #5A6B00; font-size: 0.8rem; font-weight: 700; padding: 0.3rem 0.9rem; border-radius: 20px; align-self: center; }
    ...
    .mini-poster-ph { width: 40px; height: 56px; border-radius: 6px; background: linear-gradient(135deg, rgba(210,224,3,0.15), rgba(210,224,3,0.05)); display: flex; align-items: center; justify-content: center; font-size: 1.2rem; }
    .td-title { font-weight: 600; font-size: 0.875rem; color: #0F1A35; }
    .td-center { text-align: center; }
    .year-chip { background: rgba(210,224,3,0.18); color: #5A6B00; font-size: 0.72rem; font-weight: 600; padding: 0.15rem 0.55rem; border-radius: 20px; }
...
export class WatchedComponent implements OnInit {
  ...
  rows: WatchedRow[] = []; loading = true; error = '';
  ngOnInit(): void {
    this.api.listWatched().subscribe({
      next: (d) => { this.rows = d; this.loading = false; },
      error: () => { this.error = 'Não foi possível carregar assistidos.'; this.loading = false; },
    });
  }
}
